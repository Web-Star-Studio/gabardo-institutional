...
function Test-HashExistsInDatabase {
    ...
    try {
        ...
        Write-Host "Erro ao verificar existência no banco: $($_.Exception.Message)" -ForegroundColor Yellow
        return $false
    }
}
...
function Get-SystemInventory {
    ...
    $sistema = @{
        ...
        timezone = $tz
        ultimo_boot = $ultimoBoot.ToString("o")
        uptime_dias = $uptime
        instalacao = $os.InstallDate.ToString("o")
        produto_id = $os.SerialNumber
        ativacao = if ($os.LicenseStatus -eq 1) { "Licenciado" } else { "Não licenciado / Desconhecido" }
    ...
    foreach ($pd in $physicalDisks) {
        ...
        $discos += @{
            ...
            particoes = $volumes
            status = if ($pd.HealthStatus -eq "Healthy") { "Saudável" } else { $pd.HealthStatus }
            # Novos
    ...
    $placa_mae = @{
        ...
        bios = @{
            ...
            data = if ($bios.ReleaseDate) { $bios.ReleaseDate.ToString("yyyy-MM-dd") } else { $null }
    ...
    foreach ($gpu in $videoControllers) {
        $gpuInfo = @{
            ...
            driver_data = if ($gpu.DriverDate) { $gpu.DriverDate.ToString("yyyy-MM-dd") } else { $null }
    ...
    foreach ($adapter in $adapters) {
        ...
        $rede += @{
            ...
            driver_date = if ($adapter.DriverDate) { $adapter.DriverDate.ToString("yyyy-MM-dd") } else { $null }
    ...
    try {
        $hotfixes = Get-HotFix -ErrorAction SilentlyContinue | Sort-Object InstalledOn -Descending | ForEach-Object {
            @{
                ...
                data_instalacao = if ($_.InstalledOn) { $_.InstalledOn.ToString("o") } else { $null }
            }
        }
    } catch {}

    # -------------------- LOGS DO SISTEMA (últimos 14 dias) --------------------
    $logs = @{
        system_errors = @()
        system_warnings = @()
        application_errors = @()
        ...
        contagem = @{
            system_errors_14d = 0
            system_warnings_14d = 0
            application_errors_14d = 0
            application_warnings_14d = 0
        }
    }
    try {
        $startTime = (Get-Date).AddDays(-14)
        $sysErr = Get-WinEvent -FilterHashtable @{LogName='System'; Level=2; StartTime=$startTime} -MaxEvents 50 -ErrorAction SilentlyContinue
        $logs.contagem.system_errors_14d = if ($sysErr) { $sysErr.Count } else { 0 }
        foreach ($e in $sysErr) {
            $logs.system_errors += @{
        ...
        $sysWarn = Get-WinEvent -FilterHashtable @{LogName='System'; Level=3; StartTime=$startTime} -MaxEvents 30 -ErrorAction SilentlyContinue
        $logs.contagem.system_warnings_14d = if ($sysWarn) { $sysWarn.Count } else { 0 }
        foreach ($e in $sysWarn) {
            $logs.system_warnings += @{
                time = $e.TimeCreated.ToString("o")
                id = $e.Id
                source = $e.ProviderName
                level = $e.LevelDisplayName
        ...
        $appErr = Get-WinEvent -FilterHashtable @{LogName='Application'; Level=2; StartTime=$startTime} -MaxEvents 50 -ErrorAction SilentlyContinue
        $logs.contagem.application_errors_14d = if ($appErr) { $appErr.Count } else { 0 }
        foreach ($e in $appErr) {
            $logs.application_errors += @{
                time = $e.TimeCreated.ToString("o")
                id = $e.Id
                source = $e.ProviderName
                level = $e.LevelDisplayName
        ...
        $appWarn = Get-WinEvent -FilterHashtable @{LogName='Application'; Level=3; StartTime=$startTime} -MaxEvents 30 -ErrorAction SilentlyContinue
        $logs.contagem.application_warnings_14d = if ($appWarn) { $appWarn.Count } else { 0 }
        foreach ($e in $appWarn) {
            $logs.application_warnings += @{
                time = $e.TimeCreated.ToString("o")
                id = $e.Id
                source = $e.ProviderName
                level = $e.LevelDisplayName
    ...
    try {
        $tarefas = Get-ScheduledTask -ErrorAction SilentlyContinue | Where-Object { $_.State -ne "Disabled" } | ForEach-Object {
            ...
            @{
                ...
                caminho = $_.TaskPath
                estado = $_.State.ToString()
                ultima_execucao = if ($info.LastRunTime) { $info.LastRunTime.ToString("o") } else { $null }
                proxima_execucao = if ($info.NextRunTime) { $info.NextRunTime.ToString("o") } else { $null }
    ...
    if ($logs.contagem.system_errors_14d -gt 20) {
        $avisos += "Muitos erros no Event Log System nos últimos 14 dias ($($logs.contagem.system_errors_14d))"
    }
    $status_geral = @{
        saude = if ($alertas.Count -eq 0) { "Excelente" } else { "Atenção" }
        alertas = $alertas
        avisos = $avisos
...
# LÓGICA PRINCIPAL
# ============================================================
Write-Host "`n=== Iniciando verificação de Hashkey ===" -ForegroundColor Cyan
...
while (-not $hashReady -and $attempt -lt $maxAttempts) {
    ...
    if ($existsInDb) {
        if ($isNewHash) {
            Write-Host "Hash $localHash já existe no banco. Gerando outra... (tentativa $attempt)" -ForegroundColor Yellow
            $localHash = New-LocalHash
            continue
        }
        else {
            Write-Host "Hash já existe no banco → será feito UPDATE" -ForegroundColor Cyan
            $hashReady = $true
            $action = "update"
        }
    }
    else {
        Write-Host "Hash livre no banco → será feito INSERT" -ForegroundColor Cyan
        $hashReady = $true
        $action = "insert"
    }
}
if (-not $hashReady) {
    Write-Host "Não foi possível obter uma hashkey válida após $maxAttempts tentativas." -ForegroundColor Red
    exit 1
}
# ============================================================
# COLETA + ENVIO
# ============================================================
Write-Host "`n=== Coletando inventário completo do sistema ===" -ForegroundColor Cyan
...
Write-Host "Enviando dados para a Edge Function (ação: $action)..." -ForegroundColor Cyan
try {
    ...
    if ($isNewHash) {
        Write-Host "Hashkey NÃO foi salva localmente porque o envio falhou." -ForegroundColor Yellow
    }
}